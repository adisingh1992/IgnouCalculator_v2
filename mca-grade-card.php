<?php
    ...
    if($_SERVER["REQUEST_METHOD"] === "POST" && $_SESSION["mca_grade_id"] === filter_input(INPUT_POST, "form_id", FILTER_SANITIZE_STRING)){
        ...
        if($enrolment === false || empty($program) || $program !== "MCA"){
        ...
        $form_id = md5(rand());
        $_SESSION["mca_grade_id"] = $form_id;

        $view = new Views();
        $view->render("header.html");
        $view->render("mca-grade-card.html", $form_id);
